...
def process_samples(data, bed_entries, input_vcf, outd):
    sample_vcfs = {}
    ...
    for row in data:
        ...
        if sample_id not in sample_vcfs:
            sample_vcfs[sample_id] = input_vcf
        ...
        if genotype in ['0/1'] and sv_name in bed_entries:
            ...
            output_vcf = os.path.join(output_vcf_dir, f'{sample_id}.vcf')

            sample_bed_filename = create_sample_bed_file([sv_name], bed_entries, sample_id, outd)
            change_genotypes(sample_vcfs[sample_id], sample_bed_filename, output_vcf)
            sample_vcfs[sample_id] = output_vcf

    # Copy the final modified VCF for each sample to the "change_to_hemi" folder
    for sample_id, vcf_path in sample_vcfs.items():
        output_vcf_dir = os.path.join(outd, "change_to_hemi")
        os.makedirs(output_vcf_dir, exist_ok=True)
        output_vcf = os.path.join(output_vcf_dir, f'{sample_id}_modified.vcf')
...
def main(input_vcf, genotypes_filename, bed_filename):
    outd = os.path.dirname(input_vcf)
    data = read_data(genotypes_filename)
    bed_entries = read_bed_file(bed_filename)
    process_samples(data, bed_entries, input_vcf, outd)

if __name__ == '__main__':
    if len(sys.argv) < 4:
        print("Usage: python process_samples.py <input_vcf> <genotypes_filename> <bed_filename>")
        sys.exit(1)
