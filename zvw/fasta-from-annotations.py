                ...
                if category in ['V', 'D', 'J']:
                    ...
                    if region:
                        if allele in allele_count:
                            ...
                            allele_modified = f"{allele}_EE{allele_count[allele]}"
                        else:
                            allele_count[allele] = 1
                            allele_modified = allele

                        fasta_data[category].append(f'>{allele_modified}\n{region}')

                    if category == 'V':  # Additionally process V-REGION-GAPPED
                        region_gapped = row.get('V-REGION-GAPPED', '')
                        if region_gapped:
                            if allele in allele_count:
                                allele_modified_gapped = f"{allele}_EE{allele_count[allele]}"
                            else:
                                allele_modified_gapped = allele

                            fasta_data['V_gapped'].append(f'>{allele_modified_gapped}\n{region_gapped}')
            else:
